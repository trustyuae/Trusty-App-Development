...
const icons = {
  Accepted: 'cart-outline',
  'In-process': 'check-outline',
  Shipped: 'truck-outline',
  'Order Delivered': 'map-marker-outline',
  'Order Completed': 'home-outline',
};
...
const StepperComponentTracking = ({statusTimeline}) => {
  ...
  useEffect(() => {
    ...
    setCurrentPosition(
      currentStep === -1 ? statusTimeline.length - 1 : currentStep,
    );
  ...
  const renderLabel = ({position, stepStatus, label, currentPosition}) => {
    ...
    return (
      <SafeAreaView
        style={{marginLeft: 20, alignItems: 'center', marginVertical: 30}}>
        <Text
          style={{
            color: position === currentPosition ? '#262525' : '#676766',
